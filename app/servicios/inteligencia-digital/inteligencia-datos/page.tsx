...
import { inteligenciaDatosData } from '@/data/services/inteligencia-datos'
...
const testimonials = [
  {
    quote:
      'Pasamos de adivinar a saber. En 30 segundos veo cómo está el negocio, qué clientes necesitan atención y cuánto vamos a vender el próximo mes.',
  ...
  {
    quote:
      'Teníamos datos en 6 sistemas que nunca se hablaban. FORJA los unificó en un solo dashboard. Ahora el equipo directivo toma decisiones con datos del día, no del mes pasado.',
    author: 'Diego Sarmiento',
    position: 'Director de Operaciones',
    company: 'AgroExport Quito',
    country: '🇪🇨 Ecuador',
    stars: 5,
  },
  {
    quote:
      'El modelo predictivo de churn nos alertó de 18 clientes en riesgo. Recuperamos 14. Ese solo resultado pagó todo el proyecto en 6 semanas.',
    author: 'Patricia Quispe',
    position: 'CEO',
    company: 'ServiFinanzas Perú',
    country: '🇵🇪 Perú',
    stars: 5,
  },
  {
    quote:
      'Teníamos Power BI comprado y sin usar. FORJA lo activó, lo conectó a nuestros sistemas reales y capacitó al equipo. Ahora lo usamos todos los días.',
  ...
  {
    quote:
      'El CRM que implementaron fue el antes y después de nuestro equipo comercial. Triplicamos el seguimiento de oportunidades sin contratar a nadie más.',
    author: 'Sofía Herrera',
    position: 'Directora Comercial',
    company: 'HealthTech MX',
    country: '🇲🇽 México',
    stars: 5,
  },
]
        ...
        {doubled.map((t, idx) => (
          <div
            ...
            className="flex-shrink-0 w-[340px] md:w-[380px] bg-white rounded-2xl p-6 shadow-lg
                       border-2 border-gray-100 hover:border-brand-purple hover:shadow-xl
          ...
          >
            ...
            <div className="relative mb-6">
              <Quote className="absolute -top-1 -left-1 w-6 h-6 text-brand-purple/25" />
            ...
            <div className="flex items-center gap-3 pt-4 border-t border-gray-100">
              <div
                className="w-10 h-10 rounded-full bg-gradient-to-br from-brand-purple to-brand-navy
...
// PÁGINA PRINCIPAL
// ═══════════════════════════════════════════════════════
export default function InteligenciaDatosPage() {
  const data = inteligenciaDatosData
  const [activeModule, setActiveModule] = useState(0)

  const stats = [
    { value: 18, suffix: '%', label: 'PYMEs en Colombia con dashboard activo', prefix: 'Solo ' },
    { value: 60, suffix: ' días', label: 'Para tu primer dashboard ejecutivo', prefix: '' },
    { value: 5, suffix: '', label: 'Módulos adaptables', prefix: '' },
    { value: 46, suffix: '%', label: 'PYMEs en LATAM que planean adoptar IA', prefix: '' },
  ]
  ...
  return (
    <div className="pt-[var(--header-height-mobile)] md:pt-[var(--header-height-desktop)]">
      ...
      <section className="py-12 bg-brand-navy relative overflow-hidden">
        <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-brand-purple via-brand-orange to-brand-turquoise" />
        <div className="container mx-auto px-4 md:px-8">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 max-w-5xl mx-auto">
            {stats.map((stat, i) => (
              ...
              >
                <div className="text-3xl md:text-4xl font-extrabold text-brand-purple-light mb-1 font-heading">
                  <AnimatedCounter
                    end={stat.value}
                    suffix={stat.suffix}
                    prefix={stat.prefix}
      ...
      <section className="py-16 md:py-20 bg-white relative overflow-hidden">
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-brand-purple/5 rounded-full blur-3xl translate-x-1/3 -translate-y-1/3 pointer-events-none" />
            ...
            {data.targetProfile.items.map((item, i) => (
              <motion.div
                ...
                className="group flex items-start gap-3 p-5 rounded-2xl border-2 border-gray-100 bg-white
                           hover:border-brand-purple hover:shadow-lg transition-all duration-300 cursor-default"
              >
                <div
                  className="w-8 h-8 rounded-lg bg-gradient-to-br from-brand-purple to-brand-navy
          ...
          <div className="flex items-center gap-4 mb-12 max-w-5xl mx-auto">
            <div
              className="flex-shrink-0 w-14 h-14 rounded-2xl bg-gradient-to-br from-brand-purple to-brand-navy
                            flex items-center justify-center shadow-lg shadow-purple-200/50"
            >
            ...
            <div>
              ...
              <p className="text-gray-500 text-base mt-1">
                La Inteligencia de Datos es la dimensión más débil en PYMEs de Latinoamérica.
              </p>
            </div>
          </div>
            ...
            {data.problems.map((pillar, i) => (
              <motion.div
                ...
                className="group relative bg-white rounded-2xl p-6 shadow-sm border-2 border-gray-100
                           hover:border-brand-purple hover:shadow-xl transition-all duration-300 overflow-hidden"
              >
                <div className="absolute -right-3 -top-3 text-[80px] font-black text-gray-50 leading-none pointer-events-none select-none group-hover:text-purple-50 transition-colors">
                  {String(i + 1).padStart(2, '0')}
                </div>
                <div className="relative z-10">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 rounded-xl bg-purple-50 flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform duration-300">
                      {pillar.iconComponent && (LucideIcons[pillar.iconComponent as keyof typeof LucideIcons] as React.ComponentType<{ className?: string; strokeWidth?: number }>) ? (
                        (() => {
                          ...
                          return Icon ? <Icon className="w-5 h-5 text-brand-purple" strokeWidth={2} /> : <span className="text-xl">{pillar.icon}</span>
                        })()
                      ) : (
                        <span className="text-xl">{pillar.icon}</span>
                      )}
                    </div>
                    <h3 className="font-bold text-brand-navy text-lg leading-tight group-hover:text-brand-purple transition-colors">
                      {pillar.title}
                    </h3>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {pillar.solution.map((s, si) => (
                      <span
                        key={si}
                        className="inline-flex items-center gap-1 text-xs bg-purple-50 text-brand-purple
                                   font-semibold px-3 py-1 rounded-full border border-purple-100"
                      >
      ...
      <section className="py-16 md:py-24 bg-brand-navy relative overflow-hidden">
        ...
        <div className="absolute top-0 left-0 w-[600px] h-[600px] bg-brand-purple/10 rounded-full blur-3xl pointer-events-none" />
        <div className="container mx-auto px-4 md:px-8 relative z-10">
          <div className="flex items-center gap-4 mb-12 max-w-5xl mx-auto">
            <div
              className="flex-shrink-0 w-14 h-14 rounded-2xl bg-gradient-to-br from-brand-purple to-brand-purple/60
                            flex items-center justify-center shadow-lg"
            >
              <BarChart3 className="w-7 h-7 text-white" />
            </div>
            <div>
              <p className="text-brand-purple-light font-bold text-xs uppercase tracking-widest mb-1">
                ACTO 02 · DE 03
              </p>
              <h2 className="text-3xl md:text-4xl font-extrabold text-white leading-tight">
                Qué Incluye la Inteligencia de Datos
              </h2>
              <p className="text-white/50 text-base mt-1">
                5 módulos adaptables. Diagnóstico o programa completo.
              </p>
            </div>
          </div>
              ...
              {data.components.map((comp, i) => (
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-semibold text-sm transition-all duration-200 border-2
                    ${activeModule === i
                      ? 'bg-brand-purple border-brand-purple text-white shadow-lg shadow-purple-900/30 scale-105'
              ...
              {data.components.map((comp, i) => (
                <motion.div
                  ...
                  className="bg-white/5 border border-white/10 rounded-xl p-4
                             hover:border-brand-purple/40 hover:bg-white/8 transition-all duration-200 group"
                >
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <div>
                      ...
                      <h4 className="text-white font-bold text-sm leading-tight group-hover:text-brand-purple-light transition-colors">
                        {comp.title}
                      </h4>
                    </div>
                  </div>
        ...
        <div className="container mx-auto px-4 md:px-8 relative z-10">
          <div className="flex items-center gap-4 mb-16 max-w-5xl mx-auto">
            ...
            <div>
              ...
              <h2 className="text-3xl md:text-4xl font-extrabold text-brand-navy leading-tight">
                La Síntesis: De Datos a Decisiones
              </h2>
              <p className="text-gray-500 text-base mt-1">
                Datos sin analítica son números en un cajón. Analítica sin datos es intuición disfrazada.
              </p>
            </div>
          </div>
          ...
          <div className="max-w-4xl mx-auto mb-16">
            <div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-2">
              <motion.div
                ...
                className="flex-1 text-center bg-gradient-to-br from-purple-50 to-white border-2 border-brand-purple/30 rounded-2xl p-6 shadow-md hover:shadow-xl hover:border-brand-purple transition-all duration-300"
              >
                <div className="w-12 h-12 mx-auto mb-3 bg-gradient-to-br from-brand-purple to-brand-navy rounded-xl flex items-center justify-center shadow-md">
                  <Database className="w-6 h-6 text-white" />
                </div>
                <h3 className="text-xl font-extrabold text-brand-purple mb-1">Datos</h3>
                <p className="text-gray-500 text-sm">Una sola fuente de verdad</p>
                <div className="flex flex-wrap gap-1.5 justify-center mt-3">
                  {['Dashboard', 'CRM', 'KPIs'].map((t) => (
                    <span key={t} className="text-xs bg-purple-100 text-brand-purple px-2.5 py-1 rounded-full font-semibold">
                      {t}
                    </span>
                  ))}
                </div>
              ...
              >
                ...
                <h3 className="text-xl font-extrabold text-brand-navy mb-1">Analítica</h3>
                <p className="text-gray-500 text-sm">Predictiva y en tiempo real</p>
                <div className="flex flex-wrap gap-1.5 justify-center mt-3">
                  {['BI', 'Predictivo', 'IA'].map((t) => (
              ...
              >
                ...
                <div className="relative z-10">
                  ...
                  <h3 className="text-xl font-extrabold text-white mb-1">Decisiones</h3>
                  <p className="text-white/70 text-sm">Con datos, no con intuición</p>
                  <div className="flex flex-wrap gap-1.5 justify-center mt-3">
                    {['Medible', 'Rápido', 'Confiable'].map((t) => (
      ...
      <section className="py-16 md:py-20 bg-gray-50 overflow-hidden">
        <div className="container mx-auto px-4 md:px-8 mb-10">
          <SectionHeader
            ...
            description="Empresas que pasaron de decidir con intuición a decidir con datos."
          />
        </div>
      ...
      <section className="py-16 md:py-20 bg-gradient-to-br from-brand-navy via-brand-violet to-brand-navy relative overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 w-80 h-80 bg-brand-orange/10 rounded-full blur-3xl pointer-events-none" />
        ...
        <div className="container mx-auto px-4 md:px-8 relative z-10 text-center">
          ...
          >
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold text-white mb-4 leading-tight">
              ¿Sigues Decidiendo Sin
              <br className="hidden md:block" /> Datos Reales?
            </h2>
            <p className="text-lg md:text-xl text-white/75 max-w-2xl mx-auto mb-10">
              Evaluamos tus fuentes actuales y te decimos en cuánto tiempo puedes
              tener tu primer dashboard ejecutivo. Sin costos, sin compromiso — solo{' '}
  ...
  return (
    ...
    >
      <div className="grid md:grid-cols-2 gap-8 items-start">
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {comp.deliverables.slice(0, 4).map((d, i) => (
              <span
                key={i}
                className="text-xs bg-brand-purple/20 text-brand-purple-light border border-brand-purple/30 px-3 py-1 rounded-full font-semibold"
              >
                {d}
              </span>
            ))}
        ...
        <div className="bg-brand-purple/10 border border-brand-purple/20 rounded-2xl p-6">
          <p className="text-brand-purple-light text-xs font-bold uppercase tracking-wider mb-3">
            Entregables
          </p>
          <ul className="space-y-2 text-white/90 text-sm">
            {comp.deliverables.map((d, i) => (
              <li key={i} className="flex items-center gap-2">
                <Check className="w-4 h-4 text-brand-purple-light flex-shrink-0" />
                {d}
              </li>
            ))}
          </ul>
          <Link
            href="/contacto"
            className="inline-flex items-center gap-2 text-sm font-bold text-brand-purple-light hover:text-white transition-colors group mt-4"
          >
