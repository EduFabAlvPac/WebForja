...
/**
 * FORJA DIGITAL - Formulario de Contacto Localizado
 * 
 * Versión del formulario de contacto que mantiene el contexto del país
 * y redirige a la página de gracias localizada.
 * 
 * @module app/[lc]/contacto/_components/ContactFormLocalized
 */
...
import { LocaleCode, getCountryByLocale } from '@/lib/country'
...
const LEGAL_TEXT: Record<LocaleCode, {
  law: string
  lawUrl: string
}> = {
  co: {
    law: 'Ley 1581 de 2012',
    lawUrl: 'https://www.funcionpublica.gov.co/eva/gestornormativo/norma.php?i=49981',
  },
  cl: {
    law: 'Ley 19.628',
    lawUrl: 'https://www.bcn.cl/leychile/navegar?idNorma=141599',
  },
  pe: {
    law: 'Ley 29733',
    lawUrl: 'https://www.gob.pe/institucion/pcm/normas-legales/292310-29733',
  },
  ec: {
    law: 'Ley Orgánica de Protección de Datos Personales',
    lawUrl: 'https://www.asambleanacional.gob.ec/es',
  },
  es: {
    law: 'normativa de protección de datos aplicable',
    lawUrl: '/legal/politica-datos',
  },
}
...
export function ContactFormLocalized({ locale, className }: ContactFormLocalizedProps) {
  ...
  const legalInfo = LEGAL_TEXT[locale] || LEGAL_TEXT.co
  ...
  // Establecer el código de país al montar
  useEffect(() => {
    setValue('countryCode', country.code)
  }, [country.code, setValue])

  const onSubmit = async (data: ContactFormValues) => {
    // Honeypot check
    if (data.website && data.website.length > 0) {
      console.warn('[ContactFormLocalized] Honeypot triggered')
      return
    }
    ...
    try {
      ...
      // Redireccionar a la página de gracias localizada
      router.push(`/${locale}/gracias`)
    } catch (error) {
      console.error('[ContactFormLocalized] Error:', error)
  ...
  return (
    ...
    >
      ...
      {/* Consentimiento Legal - Localizado */}
      <div className="border-t border-slate-200 pt-4">
        <div className="flex items-start gap-3">
          ...
          <label htmlFor="aceptaPoliticas" className="text-sm text-slate-600 leading-relaxed">
            Acepto la{' '}
            <a
              href={`/${locale}/legal/politica-datos`}
              target="_blank"
