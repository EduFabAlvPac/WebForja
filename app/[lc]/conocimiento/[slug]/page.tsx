/**
 * Página de artículo del Centro de Conocimiento (localizada)
 * @module app/[lc]/conocimiento/[slug]/page
 */
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `${ORG.baseUrl}/${params.lc}/conocimiento/${params.slug}` },
  }
}

export default function ArticuloLocalePage({ params }: PageProps) {
  const { lc, slug } = params
  const articulo = ARTICULOS.find((a) => a.slug === slug)
  if (!articulo) notFound()

  const basePath = ['co', 'cl', 'pe', 'ec'].includes(lc) ? `/${lc}` : ''
